...
def test_seed_catalog_has_100_valid_unique_products():
    ...
    assert len({product["name"] for product in products}) == 100
    assert len({product["slug"] for product in products}) == 100
    assert len({product["sku"] for product in products}) == 100
    assert {product["sku"] for product in products} == {
        f"DEMO-{number:03d}" for number in range(1, 101)
    }
